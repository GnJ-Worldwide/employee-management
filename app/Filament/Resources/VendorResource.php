...
class VendorResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        return $form->schema([
            ...
            Forms\Components\Section::make('Statutory Details')
                ->columns(2)
                ->schema([
                ...
                ->schema([
                    ...
                    Forms\Components\Group::make()->schema([
                        ...
                        Forms\Components\FileUpload::make('doc_user_attachment_1_path')
                            ...
                            ->disk('public')
                            ->directory('vendor-docs/attachments')
                            ->acceptedFileTypes(['application/pdf', 'image/*'])
                            ->maxSize(5120)
                            ->downloadable()
                            ->openable()
    ...
    public static function infolist(Infolist $infolist): Infolist
    {
        return $infolist->schema([
            Infolists\Components\Section::make('Basic Info')
                ->columns(2)
                ->schema([
                    Infolists\Components\TextEntry::make('name')->label('Vendor Name'),
                    Infolists\Components\TextEntry::make('contact_person')->label('Contact Person'),
                    Infolists\Components\TextEntry::make('mobile')->label('Mobile'),
                    Infolists\Components\TextEntry::make('email')->label('Email'),
                    Infolists\Components\TextEntry::make('address')->label('Address')->columnSpanFull(),
                ]),
            ...
            Infolists\Components\Section::make('Location')
                ...
                ->schema([
                    ...
                    Infolists\Components\TextEntry::make('village')->label('Village'),
                    Infolists\Components\TextEntry::make('pincode')->label('PIN Code'),
                ]),

            Infolists\Components\Section::make('Statutory Details')
                ->columns(2)
                ->schema([
                    Infolists\Components\TextEntry::make('gst_no')->label('GST Number')->copyable(),
                    Infolists\Components\TextEntry::make('pan_no')->label('PAN Number')->copyable(),
                ]),

            Infolists\Components\Section::make('Documents & Attachments')
                ->icon('heroicon-o-paper-clip')
                ->columns(2)
                ->collapsible()
                ->schema([
                    ...
                    Infolists\Components\Group::make()->schema([
                        Infolists\Components\TextEntry::make('pan_document_mode')
                            ->label('PAN Format')
                            ->badge()
                            ->formatStateUsing(fn(string $state) => $state === 'file' ? '📎 File' : '📝 Text')
                            ->color(fn(string $state) => $state === 'file' ? 'primary' : 'info'),
                        Infolists\Components\TextEntry::make('pan_document_path')
                            ->label('PAN File')
                            ->placeholder('Not uploaded')
                            ->visible(fn(Vendor $record) => $record->pan_document_mode === 'file'),
                        Infolists\Components\TextEntry::make('pan_document_text')
                            ->label('PAN Text Content')
                            ->placeholder('Not provided')
                            ->visible(fn(Vendor $record) => $record->pan_document_mode === 'text'),
                    ])->columnSpan(1),

                    // GST Certificate
                    Infolists\Components\Group::make()->schema([
                        Infolists\Components\TextEntry::make('gst_certificate_mode')
                            ->label('GST Format')
                            ->badge()
                            ->formatStateUsing(fn(string $state) => $state === 'file' ? '📎 File' : '📝 Text')
                            ->color(fn(string $state) => $state === 'file' ? 'primary' : 'info'),
                        Infolists\Components\TextEntry::make('gst_certificate_path')
                            ->label('GST File')
                            ->placeholder('Not uploaded')
                            ->visible(fn(Vendor $record) => $record->gst_certificate_mode === 'file'),
                        Infolists\Components\TextEntry::make('gst_certificate_text')
                            ->label('GST Text Content')
                            ->placeholder('Not provided')
                            ->visible(fn(Vendor $record) => $record->gst_certificate_mode === 'text'),
                    ])->columnSpan(1),

                    // User Attachment 1
                    Infolists\Components\Group::make()->schema([
                        Infolists\Components\TextEntry::make('doc_user_attachment_1_mode')
                            ->label('Attachment 1 Format')
                            ->badge()
                            ->formatStateUsing(fn(string $state) => $state === 'file' ? '📎 File' : '📝 Text')
                            ->color(fn(string $state) => $state === 'file' ? 'primary' : 'info'),
                        Infolists\Components\TextEntry::make('doc_user_attachment_1_path')
                            ->label('Attachment 1 File')
                            ->placeholder('Not uploaded')
                            ->visible(fn(Vendor $record) => $record->doc_user_attachment_1_mode === 'file'),
                        Infolists\Components\TextEntry::make('doc_user_attachment_1_text')
                            ->label('Attachment 1 Text')
                            ->placeholder('Not provided')
                            ->visible(fn(Vendor $record) => $record->doc_user_attachment_1_mode === 'text'),
                    ])->columnSpan(1),

                    // User Attachment 2
                    Infolists\Components\Group::make()->schema([
                        Infolists\Components\TextEntry::make('doc_user_attachment_2_mode')
                            ->label('Attachment 2 Format')
                            ->badge()
                            ->formatStateUsing(fn(string $state) => $state === 'file' ? '📎 File' : '📝 Text')
                            ->color(fn(string $state) => $state === 'file' ? 'primary' : 'info'),
                        Infolists\Components\TextEntry::make('doc_user_attachment_2_path')
                            ->label('Attachment 2 File')
                            ->placeholder('Not uploaded')
                            ->visible(fn(Vendor $record) => $record->doc_user_attachment_2_mode === 'file'),
                        Infolists\Components\TextEntry::make('doc_user_attachment_2_text')
                            ->label('Attachment 2 Text')
                            ->placeholder('Not provided')
                            ->visible(fn(Vendor $record) => $record->doc_user_attachment_2_mode === 'text'),
                    ])->columnSpan(1),

                    // User Attachment 3
                    Infolists\Components\Group::make()->schema([
                        Infolists\Components\TextEntry::make('doc_user_attachment_3_mode')
                            ->label('Attachment 3 Format')
                            ->badge()
                            ->formatStateUsing(fn(string $state) => $state === 'file' ? '📎 File' : '📝 Text')
                            ->color(fn(string $state) => $state === 'file' ? 'primary' : 'info'),
                        Infolists\Components\TextEntry::make('doc_user_attachment_3_path')
                            ->label('Attachment 3 File')
                            ->placeholder('Not uploaded')
                            ->visible(fn(Vendor $record) => $record->doc_user_attachment_3_mode === 'file'),
                        Infolists\Components\TextEntry::make('doc_user_attachment_3_text')
                            ->label('Attachment 3 Text')
                            ->placeholder('Not provided')
                            ->visible(fn(Vendor $record) => $record->doc_user_attachment_3_mode === 'text'),
                    ])->columnSpan(1),
                ]),
        ]);
    }
    ...
    public static function getPages(): array
    {
        return [
            ...
            // 'view' => Pages\ViewVendor::route('/{record}'), // Can uncomment if View Page exists
            'edit' => Pages\EditVendor::route('/{record}/edit'),
        ];
    }

    // Virtual Location Helpers (same as before)
